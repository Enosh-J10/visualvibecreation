...
export type ButtonVariant =
  'primary' | 'secondary' | 'ghost' | 'text' | 'icon' | 'github' | 'linkedin' | 'playstore' | 'cta';
...
export default function Button({
  ...
  const variants: Record<ButtonVariant, string> = {
    primary: 'bg-accent-cyan text-bg-primary hover:bg-[#8be5ee] active:bg-[#72cbd5] px-6',
    secondary:
    ...
    ghost: 'bg-transparent text-text-secondary hover:text-white hover:bg-white/[0.03] px-5',
    text: 'bg-transparent text-accent-cyan hover:text-white hover:underline px-0 min-h-0 py-1.5',
    icon: 'bg-transparent text-text-secondary hover:text-white hover:border-border-standard hover:bg-white/[0.02] border border-transparent h-11 w-11',
    github:
    ...
    cta: 'bg-accent-teal text-bg-primary hover:bg-[#0b7c72] active:bg-[#09635b] px-7 shadow-lg shadow-accent-teal/10',
  };
  ...
  const content = (
    <>
      ...
      {!loading &&
        ...
        variant !== 'github' &&
        variant !== 'linkedin' &&
        variant !== 'playstore' && <ArrowUpRight className="h-3.5 w-3.5 text-current shrink-0" />}
    </>
  );
