...
export function LeadParagraph({ children, className = '' }: TypographyProps) {
  return <p className={`text-lead leading-relaxed text-text-secondary ${className}`}>{children}</p>;
}

export function Body({ children, className = '' }: TypographyProps) {
  return <p className={`text-body leading-relaxed text-text-primary ${className}`}>{children}</p>;
}

export function Caption({ children, className = '' }: TypographyProps) {
  return <span className={`text-caption block ${className}`}>{children}</span>;
}

export function Overline({ children, className = '' }: TypographyProps) {
  return (
    <span className={`text-overline block uppercase tracking-widest ${className}`}>{children}</span>
  );
}
...
export function Quote({ children, author, className = '' }: TypographyProps & { author?: string }) {
  return (
    <blockquote
      className={`border-l-2 border-accent-teal pl-4 italic text-text-secondary my-4 ${className}`}
    >
      <p className="text-body leading-relaxed">“{children}”</p>
      {author && (
        <cite className="text-caption not-italic block mt-2 text-text-muted">— {author}</cite>
      )}
    </blockquote>
  );
}
...
export function HighlightText({ children, className = '' }: TypographyProps) {
  return <span className={`text-white font-semibold ${className}`}>{children}</span>;
}
...
export function GradientText({
  ...
  className = '',
  variant = 'standard',
}: TypographyProps & { variant?: 'standard' | 'teal' }) {
...
export function StatusLabel({
  ...
  className = '',
  type = 'info',
}: TypographyProps & { type?: 'info' | 'success' | 'warning' | 'error' }) {
  const borderColors = {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-mono tracking-widest uppercase border ${borderColors[type]} ${className}`}
    >
